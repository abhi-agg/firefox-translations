...
add_task(async function testSupervisedQE() {
  info("Test the Supervised QE functionality");
  ...
  let tab = await BrowserTestUtils.openNewForegroundTab(
    gBrowser,
    `${baseURL }browser_translation_qe_test.html`
  );
  ...
  is(selectedLanguage, "English", "Detected language is in English");
  ...
  // check if the translation happened and qe scores are returned
  await SpecialPowers.spawn(gBrowser.selectedBrowser, [], async () => {
    const checkTranslation = async (document, message) => {
      ...
      is(
        ...
        "Hola mundo. Eso es una prueba de testes de traducciones.",
      ...
      */
       ...
       */
       ...
       document.getElementById("OTapp").querySelectorAll("textarea")[0].value = "Hola mundo";
